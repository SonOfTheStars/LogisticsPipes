...
import logisticspipes.renderer.shaderbased.model.Face
import logisticspipes.renderer.shaderbased.model.OBJRoot
import logisticspipes.renderer.shaderbased.model.Object
import logisticspipes.renderer.shaderbased.model.fallbackMaterial
...
import net.minecraft.client.renderer.GlStateManager.setActiveTexture
...
class NewGLRenderer(val obj: OBJRoot, val textureOverrides: Map<String, String> = emptyMap()) {
    ...
    private val glTextureIds = mutableMapOf<ResourceLocation, Int>()
    ...
    fun draw(light: Int = 0xF000F0) {
        ...
        if (shader != 0) {
            ...
            putMatrix(fb, Mat4.IDENTITY)
            for (obj in obj.objects.values) {
                putMatrix(fb, translations[obj] ?: Mat4.IDENTITY)
            }
            fb.flip()
            ...
            for ((index, id) in textures.withIndex()) {
                setActiveTexture(GL_TEXTURE2 + index)
                enableTexture2D()
                bindTexture(id)
            }
    ...
    private fun putMatrix(fb: FloatBuffer, m: Mat4) {
        m.intoBuffer(fb)
    }
    ...
    private fun processFace(face: Face, objindex: Int) {
        ...
        val textureRL = (textureOverrides[face.material]
                ?: mat.diffuseTexture)?.let(::ResourceLocation)
                ?: ResourceLocation("quacklib", "textures/white.png")

        if (textureRL !in glTextureIds) {
            mc.textureManager.bindTexture(textureRL)
            val texid = mc.textureManager.getTexture(textureRL).glTextureId
        ...
        for (vert in face.vertices) {
            ...
            dataBuffer.putFloat(0f)
            dataBuffer.putFloat(mat.diffuse.red/256f)
            dataBuffer.putFloat(mat.diffuse.green/256f)
            dataBuffer.putFloat(mat.diffuse.blue/256f)
