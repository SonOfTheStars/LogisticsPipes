package logisticspipes.renderer.shaderbased.model
...
fun loadMaterialLibrary(rl: ResourceLocation): Map<String, Material>? {
    ...
    for (it in source.lines().map { it.replace(regex, "") }.map(String::trim)) {
        ...
        if (mat == null) {
            LogisticsPipes.log.warn("Ignoring line '$it' because no material is currently being defined")
            continue
        }
        when (cmd) {
            ...
            else -> LogisticsPipes.log.warn("Unrecognized/unimplemented MTL statement '$it'")
        }
    }
